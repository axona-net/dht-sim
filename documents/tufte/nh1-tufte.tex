...
% page-break points in this document. Switching to a plain pagestyle
% bypasses the letterspaced running heads but keeps the typography.
\pagestyle{plain}
\thispagestyle{plain}
...
\vspace{0.4em}
\begin{flushright}
\textit{\footnotesize An N-DHT Explainer\quad v0.3.48\quad 2026-05-05}
\end{flushright}
...
\newthought{Twenty hops} sounds fast, but each hop sends a message between two random computers somewhere on Earth. If your peers are scattered randomly across the globe, the average pair sits about half the planet apart---roughly 100~milliseconds one-way.
