...
\title{Building a Smarter Internet}
...
\newthought{Twenty hops} sounds fast, but each hop sends a message between two random computers somewhere on Earth. If your peers are scattered randomly across the globe, the average pair sits about half the planet apart---roughly 100~milliseconds for a round trip.
...
\newthought{The second idea}, \textbf{NH-1}, is the heart of the paper. It asks a different question:
...
\textbf{1. Navigate} --- pick the next hop for a lookup. NH-1 scores each candidate by combining XOR distance progress, learned weight, and observed latency. The latency factor halves the score every 100~ms---so a peer that's mathematically a \emph{bit} further but physically a \emph{lot} closer wins.

\textbf{2. Learn} --- strengthen what works. Four mechanisms run on every successful lookup:

\begin{itemize}
  \item \textbf{LTP}: every connection on the successful path gets stronger.
...
In neuroscience, the \emph{output} of a neuron---the long branching cable that delivers signals to many downstream targets---is called an \textbf{axon}. NH-1 builds axonal delivery trees:
...
\begin{axis}[
  ...
  \addplot[gray, line width=0.9pt, mark=*, mark size=1.5pt] coordinates {(1,410) (2,503) (3,548)};
  \node[font=\scriptsize, color=inkmuted, anchor=west] at (axis cs:3.05,548) {Kademlia (K-DHT)};
  ...
  % G-DHT
  \addplot[gray!70!black, line width=0.9pt, mark=*, mark size=1.5pt] coordinates {(1,268) (2,287) (3,291)};
  \node[font=\scriptsize, color=inkmuted, anchor=west] at (axis cs:3.05,291) {G-DHT};

  % NH-1 (accent)
  \addplot[rust, line width=1.4pt, mark=*, mark size=2pt] coordinates {(1,240) (2,254) (3,264)};
  \node[font=\scriptsize, color=rust, anchor=west] at (axis cs:3.05,264) {NH-1};

  % NX-17 (dark)
  \addplot[gray!50!black, line width=0.9pt, mark=*, mark size=1.5pt] coordinates {(1,215) (2,241) (3,243)};
  \node[font=\scriptsize, color=inkmuted, anchor=west] at (axis cs:3.05,243) {NX-17};

\end{axis}
\end{tikzpicture}
\caption[Latency vs.\ network size]{Global lookup latency versus network size, plotted against the Dabek $3\delta$ analytical floor (gray dashed). Kademlia \emph{worsens} as the network grows---more hops, each at full pairwise latency, each contributing fully to the total. NX-17 hugs the floor at $1.18\times$; NH-1 at $1.28\times$. The remaining 18\% overhead matches what the geometric series predicts for $\sim$4.5 hops where an ideal protocol takes 3.}
\label{fig:floor}
\end{figure*}

NH-1's predecessor, NX-17, hits \textbf{$1.18\times$ the floor} at 25{,}000 nodes. NH-1 hits \textbf{$1.28\times$}. They sit at the theoretical limit. The remaining 18\% overhead is structural---they take about 4.5 hops where an ideal protocol would take 3, and each ``extra'' hop costs about $\delta/2$, exactly as the geometric series predicts.
...
\newthought{The sharpest} demonstration is what the paper calls the \textbf{Slice World} test. The network gets cut almost in half---Eastern hemisphere on one side, Western on the other, connected only through a \emph{single node} near Hawaii. Every other cross-hemisphere connection is severed.
...
% =====================================================================
% X. THE HONEST FOOTNOTES
% =====================================================================
\section{What the Simulator Doesn't Model}

\newthought{The paper} is explicit about what \emph{isn't} measured.
...
\begin{itemize}
  ...
  \item \textbf{Bandwidth saturation.} A popular peer will get overloaded. The simulator's latency model doesn't capture this. Real systems risk ``success disasters''---everyone routes through the fastest peer until it collapses, then abandons it, then floods back when it recovers, in oscillations that are hard to dampen.
...
These get called out in a ``red team'' appendix.\sidenote{The companion red-team analysis ranks thirteen deployment-relevant gaps by severity and time-to-fix.} The paper's results show \textbf{the brain working perfectly}; the \emph{body}---actually deploying it on real internet conditions---still needs work.
