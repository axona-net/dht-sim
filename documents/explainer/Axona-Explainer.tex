...
\vspace{2em}
{\small An N-DHT Explainer \quad v0.3.51 \quad 2026-05-16\par}
\end{center}
\vfill
\null
\newpage
...
% =====================================================================
\section{How Does This Become Real Software?}
...
The trick is keeping two things separate that everyone \emph{wants} to merge: \textbf{the protocol} (the rules of routing---AP scoring, hop caching, vitality, axonal trees) and \textbf{the network} (the actual machinery that moves bytes between machines). If you tangle them together, the simulator becomes useless the moment you deploy, because the protocol code was wired into a fake network. If you keep them apart, the simulator becomes the deployment vehicle: same protocol, different network underneath.

N-DHT keeps them apart with \textbf{two contracts}.\sidenote{The contracts live at \texttt{src/contracts/DHT.js} and \texttt{src/contracts/Transport.js} in the codebase; both are abstract base classes whose every method throws \texttt{not implemented}. Concrete implementations live in the simulator (\texttt{SimulatedTransport}) and, when deployed, in production (\texttt{WebRTCTransport}).}

The first contract---call it the \textbf{DHT contract}---is what the application above sees. An app like a chat client doesn't care how routing works internally; it just wants to \emph{do things}. So the DHT exposes eight verbs: \texttt{start}, \texttt{stop}, \texttt{join}, \texttt{leave} (lifecycle); \texttt{lookup}, \texttt{subscribe}, \texttt{unsubscribe}, \texttt{publish} (the actual operations); plus \texttt{getMetrics}, \texttt{getSynaptome}, and \texttt{onEvent} for telemetry---a way for the application to \emph{watch} what the protocol is doing without being able to mess with it.

The second contract---the \textbf{Transport contract}---is what the network underneath has to provide. It's deliberately small: open a channel to a peer, close a channel, send a message and wait for the reply, send a message and don't wait, register a callback for when a peer dies, ask for a peer's measured latency. Twelve methods. That's it.

The protocol---the routing logic, the learning rules, all the brain-inspired machinery---sits in between. It calls \emph{down} into the Transport (``send this peer a message asking for its closest synapses to target X'') and emits events \emph{up} through the DHT contract (``a lookup just completed; here's the result''). It doesn't know whether the Transport beneath it is the simulator's in-process fake or a real WebRTC connection over the internet. \textbf{It can't tell the difference}, by design.
...
So the simulator is the deployment vehicle, and the plumbing on the other side now exists. The production Transport---a browser-resident node called \textbf{axona-peer}, running on WebRTC data channels---is live at \texttt{https://axona.net}. A signaling broker, \textbf{axona-bridge}, runs at \texttt{https://bridge.axona.net} and handles the WebRTC offer/answer exchange that two peers behind NATs need to find each other; it carries no application traffic and any operator can stand one up, so the rendezvous role is interchangeable. The cold-start problem---finding your first peer when you've never been on the network before---resolves through any of three \texttt{BootstrapEndpoint} variants: a rendezvous URL with a signed manifest, a QR-code-pasted pairing string for direct device-to-device pairing, or an in-process simulator pointer. Once bootstrap returns one open channel, the routing logic is unchanged---the same \texttt{lookup\_step} chain that the simulator runs.

The product name for this whole stack---the peer, the bridge, the protocol, the SDK that ships them to applications---is \textbf{Axona}. The protocol is N-DHT; the network of nodes running it is Axona. The first end-user application running on it in production is \texttt{civildefense.io}, a tap-to-report incident map built in weeks because the substrate primitives (signed posts, geographic locality, 24-hour expiry, anonymous P2P) inherit directly from this protocol layer. Source for the three live components: \texttt{github.com/axona-net/axona-peer}, \texttt{github.com/axona-net/axona-bridge}, \texttt{github.com/axona-net/dht-sim}.
