...
tail latency on million-node networks. We present \axona, a learning-adaptive
DHT and axonal publish/subscribe protocol, and report two technical
contributions and a deployed implementation. First, the \emph{Geographic DHT}
(\GDHT) embeds an S2 cell prefix in node identifiers so that XOR distance in
ID space approximates physical distance --- a simple structural change that
reduces $500$\,km regional lookup latency by $3.4\times$ over Kademlia at
$25\,000$ nodes. Second, \axona introduces \emph{vitality} --- a per-edge
score $\mathrm{weight} \times \mathrm{recency}$ in which the weight is
...
specialized rules and forty-four parameters into twelve each. The current
\axona implementation, \nhone, embodies this consolidation. On a
...
\nhone plateaus at $1.28\times$. \axona provides built-in publish/subscribe
...
alone outperforms Kademlia by $26\%$, demonstrating that the contribution
is structural rather than a consequence of locality engineering. \nhone is
the implementation currently deployed in the \axona peer-to-peer mesh
(\url{https://axona.net}); when this paper refers to \nhone, it refers to
the same code path validated in production browsers across real WebRTC
networks. \emph{Naming convention used throughout:} \axona names the
protocol described here, \NDHT (Neuromorphic DHT) names the broader
family of learning-adaptive DHT designs to which \axona belongs, and
\nhone names the specific implementation against which the empirical
results in this paper were measured.
\end{abstract}
...
nodes (\S\ref{sec:gdht}), and is the substrate on which \axona's
learning is layered.

\item \textbf{\axona (current implementation \nhone)}, a learning-adaptive
DHT layered above the Kademlia/\GDHT substrate, that consolidates the
$18$-rule, $44$-parameter \NXseventeen lineage into a $12$-rule,
$12$-parameter protocol governed by a single vitality-based admission gate
(\S\ref{sec:nh1}).
...
under churn. \axona achieves $100\%$ baseline delivery and $100\%$
...
\axona (\nhone) routes $8\%$ faster (\S\ref{sec:eval-geo}). Learning is
doing real work, not sharpening pre-existing geographic structure.
\end{enumerate}
...
\S\ref{sec:nh1} describes \axona's five operations, twelve rules, and
vitality function in detail (as embodied in \nhone). \S\ref{sec:pubsub} covers axonal pub/sub.
...
\axona inherits structurally from three foundational designs. Kademlia
...
becomes \axona's vitality recency multiplier. Pastry~\cite{rowstron2001pastry}
contributes
...
live identifier). \axona retains all of these mechanisms in some form;
...
demonstrates that self-tuning works. \axona applies the same instinct
...
override that migrates hot keys to less-loaded peers. \axona's
\S\ref{sec:future} identifies these as the canonical mitigations for
two open security and load-balancing gaps.
...
Both treat the network as something to measure and adapt to. \axona
...
The framing of \axona as a ``neuromorphic'' DHT (placing it in the
\NDHT family) is not decorative. The brain and a peer-to-peer DHT face
a precisely shared engineering problem, and the solution that biological
evolution found applies to overlay routing with no metaphorical slippage.
...
high-frequency event. \axona's \texttt{INERTIA\_DURATION} parameter
(\nhone) implements precisely this protection in routing.
...
this rule. \axona applies it not to perception but to \emph{routing}:
...
The translation from neuroscience mechanisms to \axona source-code
mechanisms (as embodied in the \nhone implementation) is direct, not
analogical (Figure~\ref{fig:mapping}, Table~\ref{tab:mapping}). Every
term has one corresponding artifact in the codebase.
...
and \axona routing-table operations as implemented in \nhone (right).
Every term on the right has one corresponding artifact in the
codebase. Arrows are shown on a representative subset to avoid visual
clutter; the full mapping is in Table~\ref{tab:mapping}.}
\label{fig:mapping}
\end{figure}

\begin{table}[t]
\centering
\caption{Direct mapping from biological mechanisms to \axona routing-table operations (in the \nhone implementation).}
\label{tab:mapping}
\small
...
level rather than runaway potentiation or silence. \axona's parameters
are presently fixed constants chosen by sweep. A metaplastic \axona,
...
% ═════════════════════════════════════════════════════════════════════════════
\section{\axona Architecture (current implementation: \nhone)}
\label{sec:nh1}

\axona is a learning-adaptive DHT layered above an unmodified Kademlia
...
gives end-to-end pseudocode for one lookup. The architecture is described
once and embodied in \nhone, the implementation against which the
empirical results in \S\ref{sec:eval} were measured.
...
\begin{table}[t]
\centering
\caption{The twelve rules of \axona (\nhone), organized by operation.}
\label{tab:rules}
\small
...
\axona's analog of Pastry's three-tier routing
...
\axona probes the top $\alpha = 2$ first hops for \emph{their} best
...
i.e.\ would move \emph{away} from the target --- \axona falls back to
...
themselves. This is the key mechanism behind \axona's partition-recovery
...
\axona tracks which peers reach \emph{out} to the local node. When an
...
The single most important consolidation in \axona is the function
...
shortcuts and never find better ones. \axona injects two sources of
...
unlimited). \axona tags a configurable \texttt{highwayPct} fraction of
...
\end{verbatim}}
\caption{End-to-end Axona (\nhone) lookup tick. All five operations
...
\begin{table}[t]
\centering
\caption{The twelve parameters of \axona (\nhone) with default values.}
\label{tab:params}
\small
\begin{tabular}{@{}lrl@{}}
\toprule
...
mechanism. \axona integrates the same mechanism directly into the
...
\label{sec:pubsub-production}

Live deployment of the axonal pub/sub layer on a
real WebRTC mesh surfaced three corner cases the simulator's abstract
network model had not exercised. Each is fixed in the protocol layer;
the production semantics diverge slightly from the simulator-textbook
description above.

\paragraph{Lazy-axon promotion.}
The simulator implementation drops a $\mathrm{publish\_k}$ that lands
at a K-closest node which does not yet hold a role for the topic ---
falling back to a routed walk that lands on another empty-role node
and gives up. In a real network, this manifests as
\emph{publish-before-subscribe lost messages}: the publisher emits
content before any subscriber has registered, the K-closest peers
silently discard it, and by the time subscribers attach the messages
are gone. The fix is to promote any K-closest receiver of an unhandled
$\mathrm{publish\_k}$ to a (childless) root role immediately and seed
the replay cache. When a subscriber later attaches, the existing
replay path serves the cache. Empty roles garbage-collect after
$\mathrm{rootGraceMs}$ (default $60$\,s) so cost remains bounded:
nodes that never accumulate subscribers shed their role on the next
refresh sweep.

\paragraph{Self-replay (\boldmath$\mathrm{lastSeenTs}$ override).}
A subtler interaction arises from the dual role of
$\mathrm{lastSeenTs}$. When a K-closest node receives a
$\mathrm{publish\_k}$ it advances
$\mathrm{lastSeenTsByTopic}[\mathrm{topicId}]$ to the latest cached
$\mathrm{publishTs}$ --- recording ``I have seen on the wire.'' If
that same node \emph{later} subscribes to the topic (a common case
since K-closest is computed from local synaptome state and may
include the node itself), the subscribe payload carries that
$\mathrm{lastSeenTs}$, and the cache filter
$\mathrm{publishTs} > \mathrm{lastSeenTs}$ excludes every cached
message --- even though those messages reside in the node's own
cache and have never been delivered to a local handler. The
application-level view (``messages delivered to my subscription'')
is logically independent of the network-level view (``publishIds my
relay has observed''). For the self-replay case
($\mathrm{subscriberId} = \mathrm{this.nodeId}$), the protocol
ignores $\mathrm{lastSeenTs}$ and dispatches the full cache directly
into the local delivery callback.

\paragraph{Multi-hop \texttt{sendDirect} (tunneled-direct).}
The K-closest replication model assumes any pair of peers can
communicate in one hop. In a real WebRTC mesh, this is only true
when both endpoints have established a direct DataChannel ---
something NAT traversal does not always achieve. The simulator
implementation of $\mathrm{sendDirect}$ is a single
$\mathrm{transport.notify}$ call; with an unbound target, the call
returns successfully but the message is silently dropped. Under
sparse mesh conditions, $4$ of $5$ fan-out attempts evaporate this
way, collapsing the K-replicated topic into whichever single peer
remains directly bound. The production fix is a transport-layer
fallback: if the target is not directly bound, wrap the message in
a $\texttt{\_\_tunneled\_direct\_\_}$ routed envelope and let the
existing \axona routing walk hop-by-hop toward the target. A small
handler at the destination unwraps the envelope and dispatches into
the same direct-handler table that a single-hop delivery would
have hit. The bridge becomes a routing hop among many rather than
a privileged relay.

\paragraph{Stress validation.}
A $100$-peer in-process stress harness exercises three scenarios in
a single Node.js process with no bridge, no WebSockets, and no
WebRTC --- pure protocol semantics. (1) One publisher, $99$
subscribers, three messages: $99/99$ full delivery, K=5 axons
plus recruited sub-axons. (2) Publish-before-subscribe: publisher
emits five messages with no subscribers, then $50$ subscribers
join late: all $50$ receive all five via replay. (3) Multi-topic:
five publishers, five topics, $95$ subscribers round-robin: every
topic delivered to every assigned subscriber, with K=5 axons per
topic spread across the network. Per-peer participation profile at
$N=100$: $62$ subscriber-only, $33$ subscriber+axon, $3$
publisher+subscriber, $1$ publisher+axon, $1$ all-roles. No idle
peers, axon-roles distribute as the K-closest set varies per
topic.
...
geographically-aware substrate on which \axona's learning is layered,
and as the comparative baseline against which \axona (\nhone) is measured.
...
\axona's contribution (\S\ref{sec:nh1}) is to layer
...
($\mathrm{geoBits} = 0$), \axona still routes $8\%$ faster than
...
\subsection{What \axona adds beyond predecessors}
...
peers at insertion time. \axona retains this property dynamically
...
explicitly. \axona consolidates the cluster hierarchy into a single
...
\paragraph{Versus SCRIBE on Pastry.} \axona integrates pub/sub directly
...
difference is that \axona's axon trees adapt under churn through routed
...
\paragraph{Warmup dependency.} \axona needs $\sim\!5\,000$ lookups
...
conditions. \axona's most important behavioral results
...
goes silent. The protocol is the thing in between. \axona is
...
\item \textbf{Messaging} (\texttt{send},
  ...
  latency for \texttt{send} but not for \texttt{notify}. \axona
...
vehicle. Years of \axona benchmark numbers --- the hop counts,
...
\paragraph{Live deployment.} As of v0.3.53 the production
Transport and BootstrapService implementations are live. The
browser-resident \axona peer (\texttt{axona-peer}) runs at
...
The product name for the protocol and the running network --- peer,
bridge, SDK --- is \axona; \nhone is the current \axona implementation,
and the code path it runs internally is the same one against which the
empirical results in \S\ref{sec:eval} were measured. The first end-user
application on the live network is \texttt{civildefense.io}, a tap-to-report
incident map whose primitives (signed posts, geographic locality,
24-hour expiry, anonymous P2P) inherit directly from this protocol layer.
...
\paragraph{Metaplastic \axona.} The natural endpoint of the previous
...
We have presented \axona, a learning-adaptive distributed hash table
and axonal publish/subscribe protocol whose current implementation,
\nhone, operates within $1.18$--$1.28\times$ the Dabek $3\delta$
analytical floor for recursive $O(\log N)$ routing on a $50\,000$-node
network. \axona consolidates eighteen specialized rules and
forty-four parameters from its predecessor \NXseventeen into twelve rules
and twelve parameters governed by a single vitality function ---
